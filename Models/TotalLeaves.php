...
class totalLeavesModel extends Model
{
    ...
    {   
        ...
        FROM leave_application, employment, leave_application_type 
        WHERE leave_application.emp_id = employment.emp_id AND leave_application.leave_type = leave_application_type.leave_type_id AND department = :department_id AND status = 2 AND leave_application.from >= :from_date AND leave_application.to <= :to_date 
        GROUP BY leave_type ORDER BY leave_application_type.leave_type_id";
        $statement = $this->pdo->prepare($sql);
        $statement->execute(array(':department_id' => $deparment_id, ':from_date' => $from, ':to_date' => $to));
